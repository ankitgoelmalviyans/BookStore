...
using BookStore.RecommendationService.Extensions;
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "BookStore.RecommendationService", Version = "v1" });

    if (!builder.Environment.IsDevelopment())
    {
        c.AddServer(new OpenApiServer { Url = "/recommendation" });
    }
        ...
        tracing
            .SetResourceBuilder(
                ResourceBuilder.CreateDefault()
                    .AddService(
                        serviceName: builder.Configuration["Otel:ServiceName"] ?? "BookStore.RecommendationService",
                        serviceVersion: "1.0.0"))
            .AddSource(BookStore.RecommendationService.Infrastructure.Observability.RecommendationServiceActivitySource.Name)
...
// Start the subscriber(s) once the app is up. Only registered in DI when Recommendations:Enabled is

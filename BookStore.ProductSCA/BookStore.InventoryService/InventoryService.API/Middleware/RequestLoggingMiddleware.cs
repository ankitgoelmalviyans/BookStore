...
namespace BookStore.InventoryService.API.Middleware
{
    ...
    /// <c>DurationMs</c> property (request-response timing). Health-probe requests are skipped.
    /// Runs inside <see cref="CorrelationIdMiddleware"/>'s LogContext scope, so each line also
    /// carries CorrelationId and TraceId.
    /// </summary>
